...
public class Partida {
	...
	public void comienzaPartida() {
		...
		daCartas();
		applyClassStart(jugador1);
		applyClassStart(jugador2);
	}
	...
	public void turnoJugador1() {
		...
		this.jugador1.setMana(Math.min(10, this.mana + (this.jugador1.getClase() == CardClass.MAGE ? 1 : 0)));
		applyTurnPassive(this.jugador1);
	...
	public void turnoJugador2() {
		if (this.jugador1.getCartas().size() == 0 && this.jugador2.getCartas().size() == 0) {
			...
			this.jugador2.setMana(Math.min(10, this.mana + (this.jugador2.getClase() == CardClass.MAGE ? 1 : 0)));
			applyTurnPassive(this.jugador2);
	...
	private void applyClassStart(Jugador j) {
		if (j == null || j.getP() == null) return;
		if (j.getClase() == CardClass.PALADIN) j.getP().setAbsorptionAmount(4.0D);
		if (j.getClase() == CardClass.ASSASSIN) j.getP().addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 200, 0));
		if (j.getClase() == CardClass.WARRIOR) j.getP().addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 200, 0));
	}
	private void applyTurnPassive(Jugador j) {
		if (j == null || j.getP() == null) return;
		switch (j.getClase()) {
		case HUNTER: j.drawACard(); break;
		case MAGE: break;
		case WARRIOR: j.getP().addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 80, 0)); break;
		case ASSASSIN: j.getP().addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 80, 1)); break;
		case PALADIN: j.getP().setAbsorptionAmount(Math.min(8.0D, j.getP().getAbsorptionAmount()+1.0D)); break;
		case NECROMANCER: if (!j.getMonstruos().isEmpty()) j.getMonstruos().get(0).setSalud(Math.min(j.getMonstruos().get(0).getSaludMax(), j.getMonstruos().get(0).getSalud()+1)); break;
		case DRUID: if (j.getP().getHealth() < j.getP().getMaxHealth()) j.getP().setHealth(Math.min(j.getP().getMaxHealth(), j.getP().getHealth()+1.0D)); break;
		default: break;
		}
	}
