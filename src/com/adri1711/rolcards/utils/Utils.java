...
public class Utils {
	public static Jugador buscaJugador(Player p, RolCards plugin) {
		...
		for (Jugador j : plugin.getJugadores()) {
			if (j != null && j.getP() != null && j.getP().getUniqueId().equals(p.getUniqueId())) {
				res = j;
				break;
			}
		}
	...
	public static void comunicaPerdedor(Jugador p, Partida part, RolCards plugin) {
		...
		if (part.getJugador1().equals(p)) {
			...
			if (part.getJugador2().getP().hasPermission("rolcards.vip")) {
				if (RolCards.getEconomy() != null) RolCards.getEconomy().depositPlayer((OfflinePlayer) player, (plugin.getPrizeAmount() * 2));
				if (plugin.getLanguage().equalsIgnoreCase("es")) {
					part.getJugador2().getP().sendMessage(
				...
				if (RolCards.getEconomy() != null) RolCards.getEconomy().depositPlayer((OfflinePlayer) player, plugin.getPrizeAmount());
				if (plugin.getLanguage().equalsIgnoreCase("es")) {
					part.getJugador2().getP().sendMessage(
		...
		if (plugin.isScoreboardEnabled()) {
			...
			if (plugin.isMySQL()) {
				...
				Score uno = obj.getScore(String.valueOf(ChatColor.GREEN.toString())
						+ (RolCards.getEconomy() != null ? RolCards.getEconomy().getBalance((OfflinePlayer) player) : 0));
	...
	public static boolean applyConfigCardEffectToPlayer(Jugador j, Jugador enemy, Card c, RolCards plugin) {
		if (!(c instanceof CreatedCard) || enemy == null) return false;
		CreatedCard cc=(CreatedCard)c;
		CardEffect e=cc.getCardEffect(); int v=cc.getCardEffectValue()==null?0:cc.getCardEffectValue();
		Player p=j.getP(), target=enemy.getP();
		p.getInventory().removeItem(new ItemStack[]{p.getItemInHand()}); p.updateInventory();
		j.setMana(Math.max(0,j.getMana()-c.getCardCost().intValue()));
		int damage=v;
		switch(e){
		case DAMAGE: target.damage(damage,p); break;
		case HEAL: healPlayer(target,v); break;
		case DRAW: for(int i=0;i<v;i++) j.drawACard(); break;
		case DAMAGE_ALL: target.damage(damage,p); for(Monster m:new ArrayList<Monster>(enemy.getMonstruos())){ if(m.getMonsterType()!=null&&!m.getMonsterType().isDead()) m.getMonsterType().damage(damage,p); } break;
		case HEAL_ALL: healPlayer(p,v); healPlayer(target,v); break;
		case MANA: j.setMana(j.getMana()+v); break;
		case STEAL_MANA: {int take=Math.min(v,enemy.getMana());enemy.setMana(enemy.getMana()-take);j.setMana(j.getMana()+take);break;}
		case POISON: target.addPotionEffect(new PotionEffect(PotionEffectType.POISON,Math.max(20,v*40),0)); break;
		case BURN: target.setFireTicks(Math.max(target.getFireTicks(),v*20)); break;
		case STUN: target.addPotionEffect(new PotionEffect(PotionEffectType.SLOW,Math.max(20,v*20),5)); target.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS,Math.max(20,v*20),0)); break;
		case SHIELD: p.setAbsorptionAmount(Math.min(40.0,p.getAbsorptionAmount()+v)); break;
		case LIFESTEAL: target.damage(damage,p); healPlayer(p,damage); break;
		case DISCARD: for(int i=0;i<v&&!enemy.getCartas().isEmpty();i++) enemy.getCartas().remove(new java.util.Random().nextInt(enemy.getCartas().size())); break;
		case RAGE: p.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE,Math.max(20,v*40),Math.max(0,v/3))); break;
		case CRITICAL: target.damage(Math.max(1,damage*2),p); break;
		case RANDOM: {CardEffect[] pool={CardEffect.DAMAGE,CardEffect.HEAL,CardEffect.DRAW,CardEffect.POISON,CardEffect.BURN,CardEffect.SHIELD};CardEffect r=pool[new java.util.Random().nextInt(pool.length)];if(r==CardEffect.DAMAGE)target.damage(damage,p);else if(r==CardEffect.HEAL)healPlayer(p,damage);else if(r==CardEffect.DRAW)j.drawACard();else if(r==CardEffect.POISON)target.addPotionEffect(new PotionEffect(PotionEffectType.POISON,damage*40,0));else if(r==CardEffect.BURN)target.setFireTicks(damage*20);else p.setAbsorptionAmount(Math.min(40.0,p.getAbsorptionAmount()+damage));break;}
		case SPAWN: return false;
		default: return false;
		}
		if(plugin.isScoreboardEnabled()){updateScoreboard(p,j,p.getName(),plugin);updateScoreboard(target,enemy,target.getName(),plugin);}
		return true;
	}
	private static void healPlayer(Player p,int amount){ if(p==null||amount<=0)return; double max=p.getMaxHealth(); p.setHealth(Math.min(max,p.getHealth()+amount)); }

	public static void doSkillToPlayer(Jugador j, Jugador enemy, Card c, RolCards plugin) {
		if (applyConfigCardEffectToPlayer(j, enemy, c, plugin)) return;
		LanguageMessages message = plugin.getMessages();
		Player p = j.getP();
